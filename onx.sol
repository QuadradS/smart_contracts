...
interface IOwnixMarket {
    ...
    event NFTListed(
        ...
        IOwnixERC1155 indexed _tokenAddress,
...
contract OwnixMarket is ERC1155Holder, Ownable, Pausable, IOwnixMarket, ReentrancyGuard {
   ...
   */
    ...
    function listNFT(uint256 _tokenId, uint256 _amount, uint256 _reservePrice, uint32 _duration) override external whenNotPaused() {
    ...
    function unlistNFT(uint256 _tokenId, uint32 _index) override external whenNotPaused() {
    ...
    function finishAuction(uint256 _tokenId, uint32 _index) override external nonReentrant() whenNotPaused() {
        ...
        if (royaltiesPerMillion > 0 && firstTimeOwner != auction.nftOwner) {
            // Calculate royalties
   ...
   */
    function _placeBid(address _bidder, uint256 _tokenId, uint32 _index, uint256 _price) public whenNotPaused() {
        Auction memory auction = auctions[_tokenId][_index];
